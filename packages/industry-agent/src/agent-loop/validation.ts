...
function requiredText(value: unknown, label: string): string {
	if (typeof value !== "string" || !value.trim())
		throw new IndustryAgentError("AGENT_EXECUTION_ERROR", `${label} must be a non-empty string`);
	return value.trim();
}

export function validateAgentLoopLimits(limits: AgentLoopLimits): AgentLoopLimits {
	const integerFields: readonly (keyof Pick<
		AgentLoopLimits,
	...
	for (const field of integerFields) {
		...
		if (!Number.isInteger(value) || value <= 0)
			throw new IndustryAgentError("INVALID_REQUEST", `${field} must be a positive integer`);
	}
	if (!Number.isFinite(limits.maxCostUsd) || limits.maxCostUsd <= 0)
		throw new IndustryAgentError("INVALID_REQUEST", "maxCostUsd must be positive");
	return limits;
}
...
export function validateAgentLoopUsage(value: unknown): AgentLoopUsage {
	...
	for (const field of fields) {
		...
		if (typeof item !== "number" || !Number.isInteger(item) || item < 0)
			throw new IndustryAgentError("AGENT_EXECUTION_ERROR", `Invalid usage field: ${field}`);
		tokens[field] = item;
	}
	if (tokens.totalTokens < tokens.inputTokens + tokens.outputTokens)
		throw new IndustryAgentError("AGENT_EXECUTION_ERROR", "totalTokens must cover inputTokens + outputTokens");
	const costUsd = value.costUsd;
	if (typeof costUsd !== "number" || !Number.isFinite(costUsd) || costUsd < 0)
		throw new IndustryAgentError("AGENT_EXECUTION_ERROR", "Invalid model/tool cost usage");
...
function validateToolAction(value: unknown): AgentLoopToolAction {
	...
	if (!isRecord(args))
		throw new IndustryAgentError("AGENT_EXECUTION_ERROR", "Planner tool args must be a JSON object");
	return {
...
export function validateAgentLoopToolExecution(
	value: unknown,
	expectedToolCallId: string,
): AgentLoopToolExecutionResult {
	if (!isRecord(value))
		throw new IndustryAgentError("AGENT_EXECUTION_ERROR", "Tool executor result must be an object");
	...
	if (rawError !== undefined) {
		if (!isRecord(rawError) || typeof rawError.code !== "string" || typeof rawError.message !== "string")
			throw new IndustryAgentError("AGENT_EXECUTION_ERROR", "Tool executor returned an invalid ToolResult error");
	...
	if (typeof value.usageAccountingComplete !== "boolean")
		throw new IndustryAgentError("AGENT_EXECUTION_ERROR", "Tool executor must declare usageAccountingComplete");
	return {
		result,
		usage: validateAgentLoopUsage(value.usage),
		usageAccountingComplete: value.usageAccountingComplete,
	};
}

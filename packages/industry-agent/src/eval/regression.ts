...
export function regressionDiffs(baseline: BenchmarkReport, candidate: BenchmarkReport, config: ReleaseGateConfig): readonly RegressionDiff[] {
	...
	for (const [metric, rule] of Object.entries(config.metricRules) as [EvalMetricName, NonNullable<ReleaseGateConfig["metricRules"][EvalMetricName]>][]) {
		...
		output.push({ metric, baseline: before, candidate: after, qualityDelta, regressed: qualityDelta < -rule.maxRegression });
	}
	return output;
}

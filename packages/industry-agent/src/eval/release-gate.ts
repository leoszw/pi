	...
	for (const component of RELEASE_COMPONENTS) {
		...
		if (before.version === after.version)
			issues.push({
				...
				component,
				message: `${component} fingerprint changed without a version bump (${after.version})`,
			});
	}
	return changed;
}
...
function validateReportCoverage(
	...
	const coverage =
		corpusIds.size === 0 ? 0 : [...corpusIds].filter((id) => observedIds.has(id)).length / corpusIds.size;
	if (unknown.length)
		issues.push({
			code: "CORPUS_INVALID",
			message: `${label} benchmark contains ${unknown.length} case ids that are not in the gated corpus`,
		});
	if (coverage < minCoverage)
		issues.push({
			code: "CORPUS_INVALID",
			message: `${label} benchmark coverage ${coverage.toFixed(4)} is below required ${minCoverage}`,
		});
	if (report.observationCount !== report.caseIds.length)
		issues.push({
			code: "CORPUS_INVALID",
			message: `${label} benchmark observationCount does not match caseIds length`,
		});
	if (observedIds.size !== report.caseIds.length)
		issues.push({ code: "CORPUS_INVALID", message: `${label} benchmark caseIds contain duplicates` });
}
...
function validateMetricSamples(
	...
	for (const [metric, rule] of Object.entries(config.metricRules) as [
		...
		if (!result || result.samples <= 0) {
			issues.push({ code: "METRIC_MISSING", metric, message: `${label} required metric ${metric} is missing` });
			continue;
		}
		if (rule.minSamples !== undefined && result.samples < rule.minSamples)
			issues.push({
				code: "METRIC_MISSING",
				metric,
				message: `${label} ${metric} has ${result.samples} samples; at least ${rule.minSamples} are required`,
			});
		...
		if (rule.min !== undefined && result.value < rule.min)
			issues.push({
				...
				metric,
				message: `${metric}=${result.value} is below minimum ${rule.min}`,
			});
		if (rule.max !== undefined && result.value > rule.max)
			issues.push({
				code: "METRIC_THRESHOLD",
				metric,
				message: `${metric}=${result.value} exceeds maximum ${rule.max}`,
			});
	}
}
...
export function evaluateReleaseGate(input: ReleaseGateInput, config: ReleaseGateConfig): ReleaseGateDecision {
	...
	for (const message of validateGoldenCorpusManifest(input.corpus, input.corpusManifest))
		issues.push({ code: "CORPUS_INVALID", message });
	...
	for (const category of config.requiredCategories) {
		if ((corpus.categoryCounts[category] ?? 0) < config.minCasesPerCategory)
			issues.push({
				code: "CORPUS_INVALID",
				message: `Required category ${category} does not meet minimum coverage`,
			});
	}
	if (input.candidateReport.corpusVersion !== input.corpus.version)
		issues.push({
			code: "CORPUS_INVALID",
			message: `Benchmark corpus version ${input.candidateReport.corpusVersion} does not match ${input.corpus.version}`,
		});
	...
	for (const caseId of config.criticalE2ECaseIds) {
		if (!corpusIds.has(caseId))
			issues.push({ code: "CORPUS_INVALID", message: `Critical E2E case ${caseId} is not present in corpus` });
		else if (!e2eObserved.has(caseId))
			issues.push({ code: "CRITICAL_E2E_FAILED", message: `Critical E2E case ${caseId} has no E2E observation` });
		else if (!e2ePassed.has(caseId))
			issues.push({ code: "CRITICAL_E2E_FAILED", message: `Critical E2E case ${caseId} failed` });
	}
	...
	if (input.mode === "COMPARE") {
		if (!input.baselineReport || !input.baselineComponents) {
			issues.push({
				code: "METRIC_MISSING",
				message: "COMPARE mode requires baseline report and component manifest",
			});
		} else {
			if (input.baselineReport.corpusVersion !== input.candidateReport.corpusVersion)
				issues.push({
					code: "CORPUS_INVALID",
					message: "Baseline and candidate benchmark reports must use the same corpus version",
				});
			...
			for (const diff of diffs)
				if (diff.regressed)
					issues.push({
						...
						metric: diff.metric,
						message: `${diff.metric} regressed from ${diff.baseline} to ${diff.candidate}`,
					});
		}
	}

...
export async function runOfflineBenchmark(
	...
	if (!Number.isInteger(concurrency) || concurrency <= 0)
		throw new Error("Offline benchmark concurrency must be a positive integer");
	...
	const worker = async (): Promise<void> => {
		while (true) {
			...
			if (observation.caseId !== testCase.id)
				throw new Error(`Executor returned caseId ${observation.caseId} for ${testCase.id}`);
	...
	if (observations.some((item) => item === undefined))
		throw new Error("Offline benchmark did not produce an observation for every corpus case");

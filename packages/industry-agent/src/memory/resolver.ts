...
const CHINESE_DIGITS: Readonly<Record<string, number>> = { 一: 1, 二: 2, 三: 3, 四: 4, 五: 5, 六: 6, 七: 7, 八: 8, 九: 9, 十: 10 };
...
function baseResolution(view: WorkingMemoryContextView): Omit<WorkingMemoryResolution, "kind"> {
	return { activeProjectId: view.effectiveProjectId, rowIds: [], entityIds: [], derivedFilters: [], requiresClarification: false };
}
...
export function resolveWorkingMemoryReference(text: string, view: WorkingMemoryContextView): WorkingMemoryResolution {
	...
	if (/把\s*(这些|刚才那些|那些)\s*导出(来)?|导出\s*(这些|刚才那些|那些)/.test(normalized)) {
		...
		if (!exportRows.length) return { ...base, kind: "EXPORT_CURRENT", requiresClarification: true, reason: "No current result set is available to export" };
		return { ...base, kind: "EXPORT_CURRENT", matchedText: normalized, rowIds: exportRows.map((row) => row.rowId), entityIds: entityIds(exportRows) };
	}

	if (/只看未完成的/.test(normalized)) {
		if (!view.lastResultSet.length) return { ...base, kind: "APPLY_FILTER", requiresClarification: true, reason: "No previous result set is available to filter" };
		return {
			...base,
			kind: "APPLY_FILTER",
			matchedText: "只看未完成的",
	...
	if (ordinal !== undefined) {
		...
		if (!selected) return { ...base, kind: "ORDINAL", matchedText: normalized, requiresClarification: true, reason: `Result set does not contain item ${ordinal}` };
		return {
			...base,
			kind: "ORDINAL",
	...
	if (/^(继续|接着来|继续处理)[。！!\s]*$/.test(normalized)) {
		...
		if (!latest) return { ...base, kind: "CONTINUE", matchedText: normalized, requiresClarification: true, reason: "No recent tool result is available to continue" };
	...
	if (/(刚才那些|这些|那些)/.test(normalized)) {
		...
		if (!referencedRows.length) return { ...base, kind: "CURRENT_SET", matchedText: normalized, requiresClarification: true, reason: "No current result set is available" };
		return { ...base, kind: "CURRENT_SET", matchedText: normalized, rowIds: referencedRows.map((row) => row.rowId), entityIds: entityIds(referencedRows) };
	}

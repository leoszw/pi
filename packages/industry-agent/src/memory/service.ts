...
import { assertPersistableSource, captureProjectId, workingMemoryScope } from "./policy.ts";
import { resolveWorkingMemoryReference } from "./resolver.ts";
import type {
	CaptureToolResultInput,
	WorkingMemoryCaptureSource,
	WorkingMemoryContextView,
	WorkingMemoryLimits,
	WorkingMemoryRow,
	WorkingMemoryRuntimeOptions,
	WorkingMemoryState,
	WorkingMemoryToolResult,
	WorkingMemoryValue,
} from "./types.ts";

const DEFAULT_LIMITS: WorkingMemoryLimits = {
	ttlMs: 24 * 60 * 60 * 1000,
	maxResolvedEntities: 100,
	maxResultRows: 200,
	maxSelectedRows: 200,
...
function rowIds(rows: readonly WorkingMemoryRow[]): Set<string> {
...
function uniqueRows(rows: readonly WorkingMemoryRow[], max: number): readonly WorkingMemoryRow[] {
	...
	const output: WorkingMemoryRow[] = [];
	for (const row of rows) {
		const id = nonEmpty(row.rowId, "rowId");
		if (seen.has(id)) continue;
		seen.add(id);
		output.push({ ...row, rowId: id });
...
function uniqueEntities(entities: readonly ResolvedEntity[], max: number): readonly ResolvedEntity[] {
	const seen = new Set<string>();
	const output: ResolvedEntity[] = [];
	for (const entity of entities) {
		const entityId = nonEmpty(entity.entityId, "entityId");
		const entityType = nonEmpty(entity.entityType, "entityType");
		const key = `${entityType}\u0000${entityId}`;
		if (seen.has(key)) continue;
		seen.add(key);
		output.push({ ...entity, entityId, entityType });
		if (output.length >= max) break;
	}
	return output;
}
...
function withoutProjectBoundSlots(state: WorkingMemoryState): WorkingMemoryState {
	const {
		resolvedEntities: _resolvedEntities,
		lastResultSet: _lastResultSet,
...
export class WorkingMemoryService {
	...
	constructor(options: WorkingMemoryRuntimeOptions) {
		...
		if (Object.values(this.limits).some((value) => !Number.isInteger(value) || value <= 0))
			throw new IndustryAgentError("MEMORY_INVALID_UPDATE", "Working memory limits must be positive integers");
	...
	async view(context: RequestContext): Promise<WorkingMemoryContextView> {
		...
		const filtered =
			state.lastResultSet !== undefined && lastResultSet.length === 0 && state.lastResultSet.value.length > 0;
	...
	async captureResolvedEntities(
		context: RequestContext,
		entities: readonly ResolvedEntity[],
		source: WorkingMemoryCaptureSource,
		...
		return this.update(context, (state, projectId, capturedAt) => ({
			...state,
			resolvedEntities: this.value(clean, persistentSource, context, projectId, capturedAt),
		}));
	}

	async captureResultSet(
		context: RequestContext,
		rows: readonly WorkingMemoryRow[],
		source: "TOOL_RESULT",
	): Promise<WorkingMemoryState> {
	...
	async setActiveFilters(
		...
		return this.update(context, (state, projectId, capturedAt) => ({
			...state,
			activeFilters: this.value(structuredClone(filters), persistentSource, context, projectId, capturedAt),
		}));
	}
	...
	async setActiveProject(
		context: RequestContext,
		projectId: string,
		source: "USER_EXPLICIT" | "SERVER_CONTEXT",
	): Promise<WorkingMemoryState> {
		const value = nonEmpty(projectId, "projectId");
		if (context.projectId && context.projectId !== value)
			throw new IndustryAgentError(
				"MEMORY_SCOPE_MISMATCH",
				"Active project cannot override the server-bound RequestContext project",
			);
		return this.update(context, (state, _projectId, capturedAt) => {
			const base =
				state.activeProject?.value && state.activeProject.value !== value ? withoutProjectBoundSlots(state) : state;
	...
	async setSelectedRows(
		context: RequestContext,
		rows: readonly WorkingMemoryRow[],
		source: "USER_EXPLICIT" | "SYSTEM_DERIVED",
	): Promise<WorkingMemoryState> {
		return this.update(context, (state, projectId, capturedAt) => {
			...
			const selected = uniqueRows(rows, this.limits.maxSelectedRows).map((row) => {
				if (!allowed.has(row.rowId))
					throw new IndustryAgentError(
						"MEMORY_INVALID_UPDATE",
						`Selected row is not in the current result set: ${row.rowId}`,
					);
				return row;
			});
	...
	async recordToolResult(context: RequestContext, input: CaptureToolResultInput): Promise<WorkingMemoryState> {
		return this.update(context, (state, projectId, capturedAt) => {
			const item: WorkingMemoryToolResult = {
				toolCallId: nonEmpty(input.toolCallId, "toolCallId"),
				toolName: nonEmpty(input.toolName, "toolName"),
				resultRefs: [...new Set(input.resultRefs)].slice(0, this.limits.maxResultRows),
			...
			return {
				...state,
				recentToolResults: [item, ...state.recentToolResults].slice(0, this.limits.maxRecentToolResults),
			};
		});
	}
	...
	private async update(
		context: RequestContext,
		mutate: (state: WorkingMemoryState, projectId: string | null, capturedAt: string) => WorkingMemoryState,
	): Promise<WorkingMemoryState> {
		...
		if (context.projectId && current.activeProject?.value !== context.projectId) {
			...
			base = {
				...base,
				activeProject: this.value(context.projectId, "SERVER_CONTEXT", context, context.projectId, capturedAt),
			};
		}
		...
		if (JSON.stringify(next).length > this.limits.maxSnapshotChars)
			throw new IndustryAgentError("MEMORY_INVALID_UPDATE", "Working memory snapshot exceeds configured size limit");
	...
	private async load(context: RequestContext): Promise<WorkingMemoryState> {
		...
		if (found && Date.parse(found.expiresAt) > this.now().getTime()) {
			this.trace(context, "LOAD", { revision: found.revision });
			return found;
		}
		...
		return {
			...
			updatedAt: now,
			expiresAt: new Date(this.now().getTime() + this.limits.ttlMs).toISOString(),
		};
	}
	...
	private value<T>(
		value: T,
		source: WorkingMemoryValue<T>["source"],
		context: RequestContext,
		projectId: string | null,
		capturedAt: string,
	): WorkingMemoryValue<T> {
		return {
	...
	private trace(
		context: RequestContext,
		event: Parameters<NonNullable<WorkingMemoryRuntimeOptions["trace"]>["record"]>[0]["event"],
		details?: JsonObject,
	): void {

...
const prepareOutput = objectSchema(
	{ proposal: { type: "object" }, uiActions: { type: "array", items: { type: "object" } } },
	["proposal", "uiActions"],
);

function prepareDefinition(
	input: Pick<ToolDefinition, "name" | "domain" | "action" | "description" | "inputSchema">,
): ToolDefinition {
	return {
		...input,
		version: "1.0.0",
...
export const MUTATION_TOOL_DEFINITIONS: readonly ToolDefinition[] = [
	...
	prepareDefinition({
		...
		inputSchema: objectSchema({ entityType, entityIds, reason: { type: "string", maxLength: 1000 } }, [
			"entityType",
			"entityIds",
		]),
	}),
	{
		...
		inputSchema: objectSchema(
			{ operationId: { type: "string", minLength: 1 }, approvalToken: { type: "string", minLength: 1 } },
			["operationId", "approvalToken"],
		),
		outputSchema: objectSchema(
			...
			["operationId", "status", "records", "verified"],
		),
		allowedEntityTypes: ["*"],
		permission: "mutation.commit",
		dataScopeRule: SCOPE_RULE,
		riskLevel: "CRITICAL",

import type { MutationOperation } from "../contracts/index.ts";
...
import type {
	...
	MutationAuditEvent,
	MutationAuditSink,
	MutationBusinessValidationInput,
	MutationBusinessValidator,
	MutationEntityPolicy,
	MutationPermissionInput,
	MutationPermissionService,
	MutationPolicyRegistry,
	MutationProposalRecord,
	MutationProposalRepository,
	MutationReadRepository,
	MutationScope,
	MutationTraceSink,
	MutationWriteGateway,
	MutationWriteSession,
} from "./types.ts";

function scopeKey(scope: MutationScope): string {
	return `${scope.userId}|${scope.tenantId}|${scope.companyId}|${scope.projectId}`;
}
...
export class InMemoryMutationPolicyRegistry implements MutationPolicyRegistry {
	...
	constructor(policies: readonly MutationEntityPolicy[] = []) {
		for (const policy of policies) this.policies.set(policy.entityType, structuredClone(policy));
	}
	get(entityType: string): MutationEntityPolicy | undefined {
		const value = this.policies.get(entityType);
...
export class InMemoryMutationPermissionService implements MutationPermissionService {
	...
	grant(scope: MutationScope, permission: string): void {
		this.grants.add(`${scopeKey(scope)}|${permission}`);
	}
	async authorize(input: MutationPermissionInput): Promise<boolean> {
		return this.grants.has(`${scopeKey(input)}|${input.permission}`);
	}
}
...
export class InMemoryMutationProposalRepository implements MutationProposalRepository {
	...
	async save(proposal: MutationProposalRecord): Promise<void> {
		if (this.proposals.has(proposal.operationId))
			throw new IndustryAgentError("REPOSITORY_ERROR", `Duplicate operationId: ${proposal.operationId}`);
	...
	async get(operationId: string): Promise<MutationProposalRecord | undefined> {
		const value = this.proposals.get(operationId);
		return value ? structuredClone(value) : undefined;
	}
	async update(proposal: MutationProposalRecord): Promise<void> {
		if (!this.proposals.has(proposal.operationId))
			throw new IndustryAgentError("REPOSITORY_ERROR", `Mutation proposal not found: ${proposal.operationId}`);
		this.proposals.set(proposal.operationId, structuredClone(proposal));
	}
}
...
export class InMemoryMutationTraceSink implements MutationTraceSink {
	...
	recordMutation(stage: string, details: Readonly<Record<string, unknown>>): void {
		this.events.push({ stage, details: structuredClone(details) });
	}
}
...
export class InMemoryMutationStore implements MutationReadRepository, MutationWriteGateway, ApprovalRepository {
	...
	constructor(seeds: readonly InMemoryMutationSeed[] = [], now: () => Date = () => new Date()) {
		...
		for (const seed of seeds)
			this.records.set(recordKey(seed.record.entityType, seed.record.entityId), {
				scope: structuredClone(seed.scope),
				record: structuredClone(seed.record),
			});
	}
	async getRecord(
		entityType: string,
		...
		if (!stored || !sameScope(stored.scope, scope) || (stored.record.deletedAt && !options.includeDeleted))
			return undefined;
		return structuredClone(stored.record);
	}
	async issue(record: ApprovalIssueRecord): Promise<void> {
		if (this.approvals.has(record.nonce))
			throw new IndustryAgentError("REPOSITORY_ERROR", "Duplicate approval nonce");
	...
	async transaction<T>(work: (session: MutationWriteSession) => Promise<T>): Promise<T> {
		...
		const workingApprovals = new Map(
			Array.from(this.approvals.entries(), ([key, value]) => [key, structuredClone(value)]),
		);
		...
		const session: MutationWriteSession = {
			...
			create: async (entityType, data, scope) => {
				...
				do {
					entityId = `mem-${nextId++}`;
				} while (workingRecords.has(recordKey(entityType, entityId)));
				const record: MutableBusinessRecord = { entityType, entityId, version: "1", data: structuredClone(data) };
				workingRecords.set(recordKey(entityType, entityId), {
			...
			update: async (entityType, entityId, patch, expectedVersion, scope) => {
				...
				const stored = workingRecords.get(key);
				if (!stored || !sameScope(stored.scope, scope) || stored.record.deletedAt)
					throw new IndustryAgentError("MUTATION_RECORD_NOT_FOUND", `Record not found: ${entityType}/${entityId}`);
				if (stored.record.version !== expectedVersion)
					throw new IndustryAgentError(
						"MUTATION_VERSION_CONFLICT",
						`Record version changed: ${entityType}/${entityId}`,
					);
				const record: MutableBusinessRecord = {
					...stored.record,
					version: bumpVersion(stored.record.version),
					data: { ...stored.record.data, ...structuredClone(patch) },
				};
				workingRecords.set(key, { scope: stored.scope, record: structuredClone(record) });
				return record;
			},
			...
			consumeApproval: async (nonce, operationId, at) => {
				...
				if (
					...
					return false;
				workingApprovals.set(nonce, { ...approval, status: "CONSUMED", consumedAt: at });
				return true;
			},
		};
		const result = await work(session);
		this.records = workingRecords;
		this.approvals = workingApprovals;
		this.nextId = nextId;
		return result;
	}
}

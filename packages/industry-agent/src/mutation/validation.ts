...
function asRecord(value: unknown, field: string): JsonObject {
	if (!value || typeof value !== "object" || Array.isArray(value)) throw new IndustryAgentError("MUTATION_VALIDATION_ERROR", `${field} must be an object`);
	return structuredClone(value as JsonObject);
}
function text(value: unknown, field: string): string {
	if (typeof value !== "string" || !value.trim()) throw new IndustryAgentError("MUTATION_VALIDATION_ERROR", `${field} must be a non-empty string`);
	return value.trim();
}
function noUnknown(args: JsonObject, allowed: readonly string[]): void {
	...
	if (extras.length) throw new IndustryAgentError("MUTATION_VALIDATION_ERROR", `Unexpected mutation arguments: ${extras.join(", ")}`);
}
function ids(value: unknown, max: number): readonly string[] {
	...
	if (new Set(normalized).size !== normalized.length) throw new IndustryAgentError("MUTATION_VALIDATION_ERROR", "entityIds must be unique");
	return normalized;
}

export function requireMutationScope(context: RequestContext): MutationScope {
	if (!context.userId || !context.tenantId || !context.companyId || !context.projectId) throw new IndustryAgentError("TOOL_SCOPE_REQUIRED", "Mutation runtime requires server-bound user, tenant, company, and project scope");
...
export function validateMutationArgs(name: string, raw: JsonObject, maxBatchSize = 100): ValidatedMutationArgs {
	switch (name) {
		case "prepare_create":
			...
			return { kind: "prepare_create", entityType: text(raw.entityType,"entityType"), values: asRecord(raw.values,"values") };
		case "prepare_update":
			noUnknown(raw, ["entityType","entityIds","patch"]);
			return { kind: "prepare_update", entityType: text(raw.entityType,"entityType"), entityIds: ids(raw.entityIds,maxBatchSize), patch: asRecord(raw.patch,"patch") };
		case "prepare_delete": {
			noUnknown(raw, ["entityType","entityIds","reason"]);
			const reason = raw.reason === undefined ? undefined : text(raw.reason,"reason");
			return { kind: "prepare_delete", entityType: text(raw.entityType,"entityType"), entityIds: ids(raw.entityIds,maxBatchSize), ...(reason ? { reason } : {}) };
		}
		case "commit_mutation":
			noUnknown(raw, ["operationId","approvalToken"]);
			return { kind: "commit_mutation", operationId: text(raw.operationId,"operationId"), approvalToken: text(raw.approvalToken,"approvalToken") };
		default:
			throw new IndustryAgentError("TOOL_NOT_FOUND", `Unsupported mutation tool: ${name}`);
	}
}

export function assertPolicyFields(policy: MutationEntityPolicy, operation: "CREATE"|"UPDATE", values: JsonObject): void {
	...
	if (allowed) {
		...
		if (invalid.length) throw new IndustryAgentError("MUTATION_VALIDATION_ERROR", `Fields are not allowed for ${operation}: ${invalid.join(", ")}`);
	}
	if (operation === "UPDATE" && policy.immutableFields) {
		...
		if (invalid.length) throw new IndustryAgentError("MUTATION_VALIDATION_ERROR", `Immutable fields cannot be updated: ${invalid.join(", ")}`);
	}
}

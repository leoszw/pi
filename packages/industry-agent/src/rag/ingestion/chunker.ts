...
export class StructureAwareRagChunker implements RagChunker {
	...
	constructor(
		version: string,
		config: RagChunkerConfig,
		tokens: RagTokenWindow = new ApproximateUnicodeTokenWindow(),
	) {
	...
	chunk(documentId: string, blocks: readonly RagExtractedBlock[]): readonly RagChunkDraft[] {
		...
		for (const block of blocks) {
			...
			const parts =
				block.type === "TABLE" || this.tokens.count(text) <= this.config.maxSectionTokens
					? [text]
			...
			parts.forEach((part, segmentIndex) => {
				...
				chunks.push({
					...
					ordinal: ordinal++,
					type: parts.length > 1 ? "WINDOW" : block.type,
					text: part,

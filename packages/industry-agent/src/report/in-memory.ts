...
export class InMemoryReportPermissionService implements ReportPermissionService {
	...
	async authorize(_input: ReportPermissionInput): Promise<boolean> { return this.allowed; }
}
...
export class InMemoryReportRendererRegistry implements ReportRendererRegistry {
	...
	get(format: ReportFormat): ReportRenderer | undefined { return this.byFormat.get(format); }
	list(): readonly ReportRenderer[] { return [...this.byFormat.values()]; }
}
...
export class StaticReportRenderer implements ReportRenderer {
	...
	private readonly handler: (input: ReportRenderInput, signal: AbortSignal) => Promise<ReportRendererOutput> | ReportRendererOutput;
	constructor(format: ReportFormat, version: string, handler: (input: ReportRenderInput, signal: AbortSignal) => Promise<ReportRendererOutput> | ReportRendererOutput) {
		this.format = format; this.version = version; this.handler = handler;
	}
...
export class InMemoryReportTraceSink implements ReportTraceSink {
	...
	record(event: ReportTraceEvent): void { this.events.push(structuredClone(event)); }
}

...
export function descendantsConfidence(count: number): BoqSearchConfidence {
	return {
		autoAccept: false,
		level: count > 0 ? "HIGH" : "NONE",
		raw: count > 0 ? 1 : 0,
		reason: "hierarchy_listing",
		recommendedResultCount: count,
	};
}
...
export function boqConfidencePolicy(
	...
	if (results.length === 0)
		return { autoAccept: false, level: "NONE", raw: 0, reason: "no_results", recommendedResultCount: 0 };
	const top1 = results[0]!;
	const top2 = results[1];
	const rerank1 = top1.scoreBreakdown.rerank;
	const rerank2 = top2?.scoreBreakdown.rerank ?? 0;
	const marginComponent = clamp01((rerank1 - rerank2) / 0.2);
	const armSupport = activeArms.length > 0 ? clamp01(top1.armSupportCount / activeArms.length) : 0;
	const raw = clamp01(
		0.3 * rerank1 + 0.25 * marginComponent + 0.2 * armSupport + 0.15 * top1.identityScore + 0.1 * top1.filterCoverage,
	);
	...
	const shortGenericGuard =
		query.queryMode === "ITEM_SHORT" && query.specTokens.length === 0 && !query.sectionName && Boolean(top2);
	...
	if (autoAccept)
		return { autoAccept: true, level: "HIGH", raw, reason: "calibrated_identity_support", recommendedResultCount: 1 };
	if (hasConflict)
		return {
			autoAccept: false,
			level: "AMBIGUOUS",
			raw,
			reason: "critical_spec_conflict",
	...
	if (ambiguity)
		return {
			...
			level: "AMBIGUOUS",
			raw,
			reason: "multiple_similar_items",
			recommendedResultCount: Math.min(3, results.length),
		};
	return {
		autoAccept: false,
		level: raw >= 0.65 ? "AMBIGUOUS" : "LOW",
		raw,
		reason: "confidence_below_auto_accept",
		recommendedResultCount: Math.min(3, results.length),
	};
}

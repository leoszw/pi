...
function tokenCoverage(query: string, candidate: string): number {
	const tokens = normalizeLedgerName(query)
		.split(/[\s>；,，。()（）/]+/)
...
export function scoreBoqBusinessFeatures(query: ParsedBoqQuery, document: BoqSearchDocument): BoqBusinessScore {
	...
	const features: Feature[] = [
		{ key: "exact_name", weight: 0.2, applicable: Boolean(normalizedQueryName), match: exactNameMatch },
		{ key: "alias_match", weight: 0.1, applicable: query.aliases.length > 0, match: aliasMatch },
		{ key: "section_match", weight: 0.1, applicable: Boolean(query.sectionName), match: sectionMatch },
		{ key: "ancestor_path_match", weight: 0.2, applicable: query.queryMode === "CONTEXT", match: pathCoverage },
		...
		{ key: "unit_match", weight: 0.05, applicable: Boolean(query.unit), match: unitMatch },
		{
			key: "leaf_preference",
			weight: 0.05,
			applicable: query.queryMode === "ITEM_SHORT" || query.queryMode === "SPEC",
			match: document.isLeaf ? 1 : 0,
		},
	...
	const positiveScore =
		applicable.reduce((sum, feature) => sum + feature.weight * Math.max(0, Math.min(1, feature.match)), 0) /
		denominator;
	const criticalSpecConflicts = countCriticalSpecConflicts(
		query.specTokens,
		document.specTokens,
		document.specFamilies,
	);
	...
	const featureMap = Object.fromEntries(
		features.map((feature) => [feature.key, feature.applicable ? feature.match : 0]),
	);
	const identityParts = [
		exactNameMatch,
		specCoverage,
		query.queryMode === "CONTEXT" ? pathCoverage : undefined,
	].filter((value): value is number => value !== undefined);
	const identityScore = identityParts.length
		? identityParts.reduce((sum, value) => sum + value, 0) / identityParts.length
		: 0;
	const structural = [query.sectionName ? sectionMatch : undefined, query.unit ? unitMatch : undefined].filter(

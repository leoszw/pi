...
import type { BoqOperation, BoqQueryCatalog, BoqQueryMode, BoqSearchRequest, BoqSearchFilters, ParsedBoqQuery } from "./types.ts";
...
const FACT_PATTERNS: readonly [RegExp, string][] = [
	[/变更后单价/, "change_after_price"], [/变更后数量/, "change_after_num"], [/变更后金额/, "change_after_amount"], [/变更次数/, "change_count"],
	[/(?<!变更后)(?:合同)?单价/, "contract_price"], [/(?<!变更后)(?:合同)?数量|工程量/, "contract_num"], [/(?<!变更后)(?:合同)?金额|合价/, "contract_amount"],
];
const ACTION_WORDS = /(?:查询|查找|查一下|搜索|帮我|请问|是多少|多少|哪些|什么|合同金额|合同数量|合同单价|变更后金额|变更后数量|变更后单价|变更次数)/g;
const UNIT_PATTERN = /(?:平方米|立方米|千克|公里|㎡|m²|m2|m³|m3|kg|km|吨|\bt\b|米|\bm\b)/i;

function resolveCode(raw: string, catalog: BoqQueryCatalog, descendantIntent: boolean): { ledgerCode?: string; ancestorCode?: string; confidence?: number } {
...
function resolveSection(raw: string, catalog: BoqQueryCatalog): { sectionId?: string; sectionName?: string; confidence?: number } {
	for (const section of catalog.sections ?? []) {
		for (const name of [section.sectionName, ...(section.aliases ?? [])]) {
			if (name && raw.includes(name)) return { sectionId: section.sectionId, sectionName: section.sectionName, confidence: 0.99 };
		}
	}
	return {};
}
...
function inferMode(raw: string, hasCode: boolean, specs: number, sectionName?: string): BoqQueryMode {
	...
	if (sectionName || /(?:章节|章|父项|下部结构|上部结构|基础|桥梁|隧道|路基|路面)/.test(raw) && compact.length > 12) return "CONTEXT";
...
function buildSemanticQuery(raw: string, code?: string): string {
	...
	return text.replace(/\s+/g, " ").replace(/^[的\s]+|[的\s]+$/g, "").trim();
}
...
export function parseBoqQuery(request: BoqSearchRequest): ParsedBoqQuery {
	...
	const operation: BoqOperation = descendantIntent && (code.ancestorCode || code.ledgerCode)
		? "LIST_DESCENDANTS"
		: factFields.length > 0 ? "FACT_LOOKUP" : "SEARCH";

...
class SandboxOperationTimeoutError extends Error {
	constructor() {
		super("Sandbox operation timed out");
		this.name = "SandboxOperationTimeoutError";
	}
}
...
async function withDeadline<T>(
	...
	const timeout = new Promise<never>((_, reject) => {
		timer = setTimeout(() => {
			controller.abort();
			reject(new SandboxOperationTimeoutError());
		}, timeoutMs);
	});
...
export class SandboxAnalysisService {
	...
	constructor(options: SandboxServiceOptions) {
		...
		if (!options.planner.version.trim() || !options.verifier.version.trim())
			throw new IndustryAgentError("SANDBOX_INVALID_REQUEST", "Planner and verifier versions must not be empty");
	}
	...
	async run(input: SandboxGoal): Promise<SandboxRunResult> {
		...
		const reportTitle =
			input.reportTitle === undefined ? "Analysis Report" : nonEmpty(input.reportTitle, "reportTitle", 500);
		if (!input.requestedReportFormats.length || input.requestedReportFormats.length > 4)
			throw new IndustryAgentError(
				"SANDBOX_INVALID_REQUEST",
				"requestedReportFormats must contain between 1 and 4 formats",
			);
		if (new Set(input.requestedReportFormats).size !== input.requestedReportFormats.length)
			throw new IndustryAgentError("SANDBOX_INVALID_REQUEST", "requestedReportFormats must be unique");
		for (const format of input.requestedReportFormats)
			if (!REPORT_FORMATS.has(format))
				throw new IndustryAgentError("SANDBOX_INVALID_REQUEST", `Unsupported report format: ${String(format)}`);
		...
		this.trace(input, runId, "ROUTE", "START", {
			availableToolCount: availableTools.length,
			plannerVersion: this.options.planner.version,
		});
		let route: SandboxRouteDecision;
		try {
			const raw = await withDeadline(
				(signal) =>
					this.options.planner.route(
						{
							goal,
			...
			const routeUsage = validateSandboxUsage(
				typeof raw === "object" && raw !== null ? (raw as Readonly<Record<string, unknown>>).usage : undefined,
			);
		...
		let schema: SandboxSchemaSnapshot;
		try {
			schema = validateSchemaSnapshot(
				input.context,
				await withDeadline(
			...
			this.trace(input, runId, "SCHEMA_DISCOVERY", "ERROR", {
				message: error instanceof Error ? error.message : String(error),
			});
			if (error instanceof IndustryAgentError) throw error;
			throw new IndustryAgentError("SANDBOX_SCHEMA_INVALID", "Schema discovery failed", { cause: error });
		}
		this.trace(input, runId, "SCHEMA_DISCOVERY", "OK", {
		...
		this.assertBeforeModel(startedAt, usage);
		this.trace(input, runId, "GENERATE", "START", {
			plannerVersion: this.options.planner.version,
			snapshotId: schema.snapshotId,
		});
		let generatedProgram: SandboxProgram;
		try {
			const raw = await withDeadline(
				(signal) =>
					this.options.planner.generate(
						{
							goal,
							context: input.context,
							schema,
			...
			this.trace(input, runId, "GENERATE", "ERROR", {
				message: error instanceof Error ? error.message : String(error),
				totalTokens: usage.totalTokens,
				costUsd: usage.costUsd,
			});
			...
			throw new IndustryAgentError(
				"SANDBOX_USAGE_ACCOUNTING_INCOMPLETE",
				"Sandbox program generation failed before usage could be trusted",
				{ cause: error },
			);
		}
		this.assertAfterModel(startedAt, usage);
		this.trace(input, runId, "GENERATE", "OK", {
			programId: generatedProgram.programId,
			queryCount: generatedProgram.queries.length,
			totalTokens: usage.totalTokens,
			costUsd: usage.costUsd,
		});
		...
		try {
			...
			this.trace(input, runId, "STATIC_VALIDATE", "ERROR", {
				message: error instanceof Error ? error.message : String(error),
			});
			throw error;
		}
		this.trace(input, runId, "STATIC_VALIDATE", "OK", {
			programId: program.programId,
			queryCount: program.queries.length,
		});
		...
		const dataAccess: SandboxReadCapability = {
			read: async (queryId: string) => {
				...
				if (!query)
					throw new IndustryAgentError("SANDBOX_BROKER_REJECTED", `Sandbox requested unknown queryId: ${queryId}`);
				if (brokerResults.has(queryId))
					throw new IndustryAgentError(
						"SANDBOX_BROKER_REJECTED",
						`Sandbox attempted to read query more than once: ${queryId}`,
					);
				if (brokerCalls >= this.limits.maxBrokerCalls)
					throw new IndustryAgentError(
						"SANDBOX_LIMIT_EXCEEDED",
						`Sandbox exceeded max broker calls ${this.limits.maxBrokerCalls}`,
					);
				await this.requirePermission(input, "sandbox.data.read", runId);
				brokerCalls += 1;
				...
				try {
					...
					const raw = await withDeadline(
						(signal) =>
							this.options.broker.executeReadOnly(
								{
									context: input.context,
									schema,
									query,
									maxRows: this.limits.maxRowsPerQuery,
									timeoutMs: brokerTimeout,
								},
								signal,
							),
						brokerTimeout,
					...
					if (brokerRows > this.limits.maxTotalBrokerRows)
						throw new IndustryAgentError(
							"SANDBOX_LIMIT_EXCEEDED",
							`Sandbox exceeded max total broker rows ${this.limits.maxTotalBrokerRows}`,
						);
					brokerChars += JSON.stringify({ columns: result.columns, rows: result.rows }).length;
					if (brokerChars > this.limits.maxTotalBrokerChars)
						throw new IndustryAgentError(
							"SANDBOX_LIMIT_EXCEEDED",
							`Sandbox exceeded max total broker characters ${this.limits.maxTotalBrokerChars}`,
						);
					brokerResults.set(queryId, result);
					this.trace(input, runId, "BROKER_READ", "OK", {
						queryId,
					...
					this.trace(input, runId, "BROKER_READ", "ERROR", {
						queryId,
						message: error instanceof Error ? error.message : String(error),
					});
					if (error instanceof IndustryAgentError) throw error;
					throw new IndustryAgentError("SANDBOX_BROKER_REJECTED", `Read-only broker failed for ${queryId}`, {
						cause: error,
					});
				}
			},
		...
		this.trace(input, runId, "SANDBOX_START", "START", {
			programId: program.programId,
			permittedQueryCount: program.queries.length,
		});
		let executed: SandboxExecutorResult;
		try {
			executed = await withDeadline(async (signal) => {
			...
			this.trace(input, runId, "SANDBOX_END", "ERROR", {
				message: error instanceof Error ? error.message : String(error),
			});
			if (error instanceof IndustryAgentError) throw error;
			if (error instanceof SandboxOperationTimeoutError)
				throw new IndustryAgentError("SANDBOX_EXECUTION_FAILED", "Sandbox execution timed out", { cause: error });
		...
		let verification: SandboxVerificationDecision;
		try {
			const raw = await withDeadline(
				(signal) =>
			...
			const verifierUsage = validateSandboxUsage(
				typeof raw === "object" && raw !== null ? (raw as Readonly<Record<string, unknown>>).usage : undefined,
			);
			usage = addSandboxUsage(usage, verifierUsage);
			verification = validateVerificationDecision(raw);
		} catch (error) {
			this.trace(input, runId, "VERIFY", "ERROR", {
				message: error instanceof Error ? error.message : String(error),
			});
			if (error instanceof IndustryAgentError) throw error;
			throw new IndustryAgentError(
				"SANDBOX_USAGE_ACCOUNTING_INCOMPLETE",
				"Sandbox verification failed before usage could be trusted",
				{ cause: error },
			);
		}
		this.assertAfterModel(startedAt, usage);
		this.trace(input, runId, "VERIFY", "OK", {
		...
		if (verification.kind === "MUTATION_REQUIRED") {
			...
			this.trace(input, runId, "MUTATION_PREPARE", "START", {
				operation: recommendation.operation,
				entityType: recommendation.entityType,
				targetCount: recommendation.targetEntityIds.length,
			});
			const mutationPrepare = await this.options.mutation.prepare({ context: input.context, recommendation });
			this.trace(input, runId, "MUTATION_PREPARE", "OK", {
				operationId: mutationPrepare.proposal.operationId,
				digest: mutationPrepare.proposal.digest,
			});
		...
		try {
			...
			this.trace(input, runId, "REPORT_BUILD", "ERROR", {
				message: error instanceof Error ? error.message : String(error),
			});
			throw error;
		}
		this.trace(input, runId, "REPORT_BUILD", "OK", {
	...
	private async requirePermission(
		input: SandboxGoal,
		permission: Parameters<SandboxServiceOptions["permissions"]["authorize"]>[0]["permission"],
		runId: string,
	): Promise<void> {
		let allowed: boolean;
		try {
			allowed = await this.options.permissions.authorize({ context: input.context, permission });
		} catch (error) {
			this.trace(input, runId, "AUTHORIZE", "ERROR", {
				permission,
				message: error instanceof Error ? error.message : String(error),
			});
			throw error;
	...
	private assertBeforeModel(startedAt: number, usage: SandboxUsage): void {
		if (usage.totalTokens >= this.limits.maxTotalTokens)
			throw new IndustryAgentError("SANDBOX_LIMIT_EXCEEDED", "Sandbox token budget exhausted");
		if (usage.costUsd >= this.limits.maxCostUsd)
			throw new IndustryAgentError("SANDBOX_LIMIT_EXCEEDED", "Sandbox cost budget exhausted");
		if (this.nowMs() - startedAt >= this.limits.maxDurationMs)
			throw new IndustryAgentError("SANDBOX_LIMIT_EXCEEDED", "Sandbox total duration exhausted");
	}

	private assertAfterModel(startedAt: number, usage: SandboxUsage): void {
		if (usage.totalTokens > this.limits.maxTotalTokens)
			throw new IndustryAgentError("SANDBOX_LIMIT_EXCEEDED", "Sandbox token budget exceeded");
		if (usage.costUsd > this.limits.maxCostUsd)
			throw new IndustryAgentError("SANDBOX_LIMIT_EXCEEDED", "Sandbox cost budget exceeded");
		if (this.nowMs() - startedAt > this.limits.maxDurationMs)
			throw new IndustryAgentError("SANDBOX_LIMIT_EXCEEDED", "Sandbox total duration exceeded");
	}

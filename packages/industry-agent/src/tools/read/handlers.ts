...
export async function executeReadHandler(context: ReadToolHandlerContext, args: ValidatedReadArgs): Promise<unknown> {
	...
	switch (args.kind) {
		case "search_engineering_positions":
			return options.engineeringSearch.search({
				query: args.query,
				projectId: scope.projectId,
				...(args.topK === undefined ? {} : { topK: args.topK }),
				...(args.leafOnly === undefined ? {} : { leafOnly: args.leafOnly }),
		...
		case "search_boq": {
			...
			return options.boqSearch.search({
				query: args.query,
				projectId: scope.projectId,
				catalog,
				...(args.topK === undefined ? {} : { topK: args.topK }),
		...
		case "query_quantity": {
			const defaultFields =
				args.entityType === "ENGINEERING_POSITION"
					? (["design_quantity", "use_quantity"] as const)
					: (["contract_num", "change_after_num"] as const);
			const value = await options.dataRepository.queryQuantity(
				args.entityType,
				args.entityId,
				args.fields.length ? args.fields : defaultFields,
				scope,
			);
			return value ?? notFound("quantity record", args.entityId);
		}
		case "list_project_documents": {
			const documents = await options.dataRepository.listProjectDocuments(scope, {

...
export interface ScopedReadSeed<T> {
	scope: ReadToolScope;
	value: T;
}
...
export class InMemoryReadDataRepository implements ReadDataRepository {
	...
	async getEngineeringPosition(
		engineeringId: string,
		scope: ReadToolScope,
	): Promise<EngineeringPositionReadRecord | undefined> {
		return this.engineering.find((item) => sameScope(item.scope, scope) && item.value.engineeringId === engineeringId)
			?.value;
	}
	...
	async queryQuantity(
		...
		const row = this.quantities.find(
			(item) =>
				sameScope(item.scope, scope) && item.value.entityType === entityType && item.value.entityId === entityId,
		)?.value;
		if (!row) return undefined;
		const values = Object.fromEntries(
			fields.filter((field) => field in row.values).map((field) => [field, row.values[field]]),
		);
		return { ...row, values };
	}
...
export class StaticBoqCatalogProvider implements BoqCatalogProvider {
	...
	constructor(catalogs: readonly { scope: ReadToolScope; catalog: BoqQueryCatalog }[]) {
		this.catalogs = catalogs;
	}
	async getCatalog(scope: ReadToolScope): Promise<BoqQueryCatalog> {
		return (
			this.catalogs.find((item) => sameScope(item.scope, scope))?.catalog ?? {
				knownCodes: new Set(),
				knownAncestorCodes: new Set(),
			}
		);
	}
}

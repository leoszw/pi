...
describe("agent loop mutation safety", () => {
	it("blocks confirmation-required critical tools", async () => {
		const planner = new ScriptedAgentLoopPlanner("p1", [
			{
				...
				action: {
					...
					toolVersion: "1.0.0",
					args: { operationId: "op", approvalToken: "token" },
					purpose: "commit",
				},
				usage: zeroUsage(),
			},
		...
		const result = await new AgentLoopService({
			...
			tools: new InMemoryAgentLoopToolCatalog([commitTool()]),
			executor,
			limits: limits(),
		}).run({ goal: "update", context: context() });
	...
	it("allows dry-run mutation prepare but blocks non-dry-run mutation even if misconfigured non-critical", async () => {
		...
		const planner = new ScriptedAgentLoopPlanner("p1", [
			{
				kind: "TOOL",
				action: { toolName: "unsafe_update", toolVersion: "1.0.0", args: {}, purpose: "unsafe" },
				usage: zeroUsage(),
			},
		]);
		const executor = new RecordingAgentLoopToolExecutor(() => {
		...
		const result = await new AgentLoopService({
			planner,
			verifier: new ScriptedAgentLoopVerifier("v1", []),
			tools: new InMemoryAgentLoopToolCatalog([direct]),
			executor,
			limits: limits(),
		}).run({ goal: "update", context: context() });
		expect(result.status).toBe("CONFIRMATION_REQUIRED");

...
describe("agent loop replan", () => {
	it("replans after verification and stays bounded", async () => {
		const action = (purpose: string) => ({
			kind: "TOOL" as const,
			action: { toolName: "search_boq", toolVersion: "1.0.0", args: {}, purpose },
			usage: zeroUsage(),
		});
		const planner = new ScriptedAgentLoopPlanner("p1", [action("first"), action("second")]);
		const verifier = new ScriptedAgentLoopVerifier("v1", [
			{ kind: "REPLAN", feedback: "need another query", usage: zeroUsage() },
			{ kind: "SATISFIED", answer: "ok", usage: zeroUsage() },
		]);

...
describe("agent loop server scope", () => {
	it("rejects a planner scope mismatch before tool execution", async () => {
		const planner = new ScriptedAgentLoopPlanner("p1", [{ kind: "TOOL", action: { toolName: "search_boq", toolVersion: "1.0.0", args: { projectId: "p-other" }, purpose: "q" }, usage: zeroUsage() }]);
		const executor = new RecordingAgentLoopToolExecutor(() => { throw new Error("must not execute"); });
		const result = await new AgentLoopService({ planner, verifier: new ScriptedAgentLoopVerifier("v1", []), tools: new InMemoryAgentLoopToolCatalog([readTool()]), executor, limits: limits() }).run({ goal: "g", context: context() });
	...
	it("strips even matching scope args so downstream scope comes only from RequestContext", async () => {
		const planner = new ScriptedAgentLoopPlanner("p1", [{ kind: "TOOL", action: { toolName: "search_boq", toolVersion: "1.0.0", args: { projectId: "p1" }, purpose: "q" }, usage: zeroUsage() }]);

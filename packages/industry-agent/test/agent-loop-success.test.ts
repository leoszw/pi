...
describe("agent loop success", () => {
	it("plans, acts, verifies and preserves server request context", async () => {
		const planner = new ScriptedAgentLoopPlanner("planner-v1", [
			{
				kind: "TOOL",
				action: { toolName: "search_boq", toolVersion: "1.0.0", args: {}, purpose: "find boq" },
				usage: usage(10),
			},
		]);
		const verifier = new ScriptedAgentLoopVerifier("verifier-v1", [
			{ kind: "SATISFIED", answer: "done", usage: usage(5) },
		]);
		const executor = new RecordingAgentLoopToolExecutor((invocation) => ({
			result: { toolCallId: invocation.toolCallId, ok: true, data: { rows: [] } },
			usage: zeroUsage(),
		...
		const ctx = context();
		const result = await service.run({ goal: "find boq", context: ctx });

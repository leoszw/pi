...
describe("agent loop tool failure", () => {
	it("lets verifier replan a normal metered ToolResult failure without automatic retry", async () => {
		const planner = new ScriptedAgentLoopPlanner("p1", [
			{
				kind: "TOOL",
				action: { toolName: "search_boq", toolVersion: "1.0.0", args: {}, purpose: "query" },
				usage: zeroUsage(),
			},
			{ kind: "FINISH", answer: "fallback answer", usage: zeroUsage() },
		]);
		const verifier = new ScriptedAgentLoopVerifier("v1", [
			{ kind: "REPLAN", feedback: "tool failed; use fallback", usage: zeroUsage() },
		]);
		const executor = new RecordingAgentLoopToolExecutor((invocation) => ({
			result: { toolCallId: invocation.toolCallId, ok: false, error: { code: "UPSTREAM", message: "down" } },
			usage: zeroUsage(),

...
describe("BOQ metrics", () => {
	it("computes exact recall ambiguity and latency metrics", () => {
		const metrics = calculateBoqBenchmarkMetrics([
			{
				expectedLedgerIds: ["a"],
				rankedLedgerIds: ["a"],
				operation: "SEARCH",
				queryMode: "CODE",
				codeExactExpected: true,
				criticalSpecConflictTop1: false,
				autoAccept: true,
				autoAcceptCorrect: true,
				latencyMs: 10,
			},
			{
				expectedLedgerIds: ["b"],
				...
				criticalSpecConflictTop1: false,
				autoAccept: false,
				autoAcceptCorrect: false,
				latencyMs: 20,
			},
		]);
		expect(metrics.codeExactAccuracy).toBe(1);

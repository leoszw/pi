...
import { buildBoqFilters, parseBoqQuery } from "../src/retrieval/boq/query-parser.ts";

describe("OpenSearch BOQ backend", () => {
	it("builds project-scoped code exact query", async () => {
		let body: Readonly<Record<string, unknown>> = {};
		const backend = new OpenSearchBoqRetrievalBackend({
			indexName: "boq_ledger_v1",
			transport: {
				async search(_index, input) {
					body = input;
					return { hits: { hits: [] } };
				},
			},
		});
		const parsed = parseBoqQuery({
			query: "403-2-a",
			projectId: "670701177435987968",
			catalog: { knownCodes: new Set(["403-2-a"]), knownAncestorCodes: new Set() },
		});
	...
	it("uses ef_search for dense arms", async () => {
		...
		expect(JSON.stringify(body)).toContain("ef_search");
		expect(JSON.stringify(body)).toContain("160");
	});
});

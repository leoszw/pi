...
const hierarchy = {
	entityIds: ["root", row.engineeringId],
	names: ["冯家沟大桥右幅", "0号台盖梁"],
	depth: 2,
	incomplete: false,
} as const;
...
describe("EngineeringIndexingService", () => {
	it("reuses vectors when embedding input hash and index version are unchanged", async () => {
		const preview = buildEngineeringSearchDocument({
			row,
			hierarchy,
			aliases: [],
		...
		const service = new EngineeringIndexingService({
			...
			state: {
				async getState() {
					return { embeddingInputHash: preview.embeddingInputHash, indexVersion: "v1" };
				},
			},
			writer: {
				async upsert(document, options) {
					writes.push({ document, preserveExistingVectors: options.preserveExistingVectors });
				},
				async delete() {},
			},
	...
	it("embeds both vector texts when the hash changes and deletes source tombstones", async () => {
		...
		const tombstone = {
			...row,
			engineeringId: "470359026153164801",
			isDeleted: true,
			updateTime: "2026-09-11T10:01:00.000Z",
		};

...
describe("phase 11 critical E2E gate", () => {
	it("blocks when a required critical workflow fails even if aggregate E2E is high", () => {
		...
		const decision = evaluateReleaseGate({ corpus, corpusManifest: buildGoldenCorpusManifest(corpus), mode: "ESTABLISH_BASELINE", candidateReport: { ...report, e2ePassedCaseIds: report.e2ePassedCaseIds.filter((id) => id !== criticalId) }, candidateComponents: components() }, config);

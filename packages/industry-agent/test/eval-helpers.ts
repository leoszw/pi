...
export function smallCorpus(perCategory = 2): GoldenCorpus {
	return {
		...
		cases: EVAL_CATEGORIES.flatMap((category) =>
...
export function gateConfig(perCategory = 2): ReleaseGateConfig {
	const metricRules: Partial<
		Record<EvalMetricName, { min?: number; max?: number; minSamples?: number; maxRegression?: number }>
	> = {};
...
export function fullReport(overrides: Partial<Record<EvalMetricName, number>> = {}): BenchmarkReport {
	...
	for (const [metric, value] of Object.entries(overrides) as [EvalMetricName, number][])
		metrics[metric] = { value, samples: 100 };
	const caseIds = smallCorpus().cases.map((item) => item.id);
	return {
		benchmarkVersion: "bench-v1",
		corpusVersion: "test-corpus-v1",
		generatedAt: "2026-09-12T00:00:00.000Z",
		observationCount: caseIds.length,
		caseIds,
		e2eObservedCaseIds: caseIds,
		e2ePassedCaseIds: caseIds,
		metrics,
	};
}
...
export function components(version = "v1", fingerprint = "fp1"): ReleaseComponentManifest {
	return {
		...
		normalizer: { version, fingerprint },
		embedding: { version, fingerprint },
		index: { version, fingerprint },
		rrf: { version, fingerprint },
		reranker: { version, fingerprint },
		toolSchema: { version, fingerprint },
	};
}

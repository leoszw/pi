...
describe("phase 11 metrics", () => {
	it("computes the first-phase metric family", () => {
		const report = computeBenchmarkReport([{
			...
			mentions: { gold: [{ start: 0, end: 3, label: "BOQ" }], predicted: [{ start: 0, end: 3, label: "BOQ" }] },
			...
			tokens: { expected: { llmCalls: 1, inputTokens: 10, outputTokens: 5, cachedTokens: 2, totalTokens: 17 }, accounted: { llmCalls: 1, inputTokens: 10, outputTokens: 5, cachedTokens: 2, totalTokens: 17 } },
			e2e: { success: true, clarified: false, manualStepsBaseline: 5, manualStepsActual: 2 },
		}], { benchmarkVersion: "b1", corpusVersion: "c1", intentCandidateK: 3, ragK: 10, generatedAt: "2026-09-12T00:00:00.000Z" });

...
describe("phase 11 release gate", () => {
	...
	it("requires a version bump when a gated component fingerprint changes", () => {
		...
		expect(decision.issues.some((item) => item.code === "VERSION_BUMP_REQUIRED" && item.component === "prompt")).toBe(
			true,
		);
	});
		...
		const decision = evaluateReleaseGate(
			{
				...
				mode: "COMPARE",
				corpus: smallCorpus(),
				candidateReport: fullReport({ ragNdcgAtK: 0.9 }),
				candidateComponents: candidate,
				baselineReport: fullReport({ ragNdcgAtK: 0.99 }),
				baselineComponents: baseline,
			},
			gateConfig(),
		);
		expect(decision.status).toBe("BLOCK");
		expect(decision.issues.some((item) => item.code === "METRIC_REGRESSION" && item.metric === "ragNdcgAtK")).toBe(
			true,
		);
	});
});

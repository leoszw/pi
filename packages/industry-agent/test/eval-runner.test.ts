...
describe("phase 11 offline runner", () => {
	it("executes every case and preserves corpus order", async () => {
		...
		const report = await runOfflineBenchmark(corpus, { execute: async (testCase) => ({ caseId: testCase.id, e2e: { success: true, clarified: false, manualStepsBaseline: 3, manualStepsActual: 1 } }) }, { benchmarkVersion: "b1", intentCandidateK: 3, ragK: 10, concurrency: 4, generatedAt: "2026-09-12T00:00:00.000Z" });

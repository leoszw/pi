...
export function context(overrides: Partial<RequestContext> = {}): RequestContext {
	return { traceId: "trace-1", requestId: "req-1", conversationId: "conv-1", userId: "u1", tenantId: "t1", companyId: "c1", projectId: "p1", createdAt: "2026-09-12T00:00:00.000Z", ...overrides };
}

export const observations: ImageObservationBundle = {
	modelVersion: "vision-v1",
	observations: [
		{ observationId: "o1", kind: "FIELD", label: "负责人", value: "张三", confidence: 0.96, bbox: { x: 0.1, y: 0.1, width: 0.2, height: 0.1 } },
...
export const resolution: ImageEntityResolution = {
	...
	matches: [{ entity: { entityId: "E1", entityType: "ENGINEERING_POSITION", canonicalName: "1#墩", confidence: 0.99 }, scope: { tenantId: "t1", companyId: "c1", projectId: "p1" }, evidenceObservationIds: ["o2"] }],
	ambiguities: [],
};
...
export function prepareResult(): MutationPrepareResult {
	return {
		proposal: {
			operationId: "op-1", traceId: "trace-1", requestId: "req-1", operation: "UPDATE", entityType: "ENGINEERING_POSITION",
			scope: { userId: "u1", tenantId: "t1", companyId: "c1", projectId: "p1" },
			targets: [{ entityId: "E1", recordVersion: "v1", before: { owner: "李四" }, after: { owner: "张三" } }],
			recordVersion: "batch-v1", digest: "digest-1", affectedCount: 1, representativeSamples: [], diff: [], status: "PREPARED", createdAt: "2026-09-12T00:00:00.000Z",
		},
...
export class RecordingPreparer implements ImageMutationPreparer {
	...
	async prepare(input: Parameters<ImageMutationPreparer["prepare"]>[0]): Promise<MutationPrepareResult> { this.calls.push(input.proposal); return prepareResult(); }
}
...
export function service(options: {
	...
	const instance = new ImageInputService({
		...
		actionProposer: new StaticImageActionProposer("action-v1", options.proposal === undefined ? updateProposal : (options.proposal ?? undefined)),
		policies: new StaticImageActionPolicyRegistry([{
			entityType: "ENGINEERING_POSITION", allowedOperations: ["CREATE", "UPDATE", "DELETE"], minPrepareConfidence: 0.9, maxTargets: 10,
			fields: {

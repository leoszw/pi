...
const policy = {
	...
	allowedCreateFields: ["name", "status"],
	allowedUpdateFields: ["name", "status"],
	immutableFields: ["id"],
	softDelete: true as const,
	maxBatchSize: 10,
};
function invocation(name: string, args: JsonObject, ctx = context): ToolInvocation {
	return { toolCallId: `tc-${name}`, toolName: name, toolVersion: "1.0.0", args, context: ctx };
}
function setup() {
	let id = 0;
	...
	const store = new InMemoryMutationStore(
		[
			{
				scope,
				record: { entityType: "ISSUE", entityId: "i1", version: "1", data: { name: "Old", status: "OPEN" } },
			},
			{
				scope,
				record: { entityType: "ISSUE", entityId: "i2", version: "7", data: { name: "Two", status: "OPEN" } },
			},
		],
		() => new Date("2026-09-12T00:00:00.000Z"),
	);
	const permissions = new InMemoryMutationPermissionService();
	for (const p of ["mutation.prepare", "mutation.commit", "issue.create", "issue.update", "issue.delete"])
		permissions.grant(scope, p);
...
describe("MutationRuntime", () => {
	it("prepare does not mutate and commit requires trusted UI approval", async () => {
		const x = setup();
		const prepared = await x.runtime.execute(
			invocation("prepare_update", { entityType: "ISSUE", entityIds: ["i1"], patch: { status: "DONE" } }),
		);
		expect(prepared.ok).toBe(true);
		expect((await x.store.getRecord("ISSUE", "i1", scope))?.version).toBe("1");
		const operationId = (prepared.data as { proposal: { operationId: string } }).proposal.operationId;
		const denied = await x.runtime.execute(
			invocation("commit_mutation", { operationId, approvalToken: "not-approved" }),
		);
		expect(denied.ok).toBe(false);
		expect(denied.error?.code).toBe("MUTATION_APPROVAL_REQUIRED");
	});
	it("commits only the exact approved digest and rejects replay", async () => {
		...
		const operationId = (prepared.data as { proposal: { operationId: string } }).proposal.operationId;
		const approval = await x.runtime.approve(operationId, context, true);
		const committed = await x.runtime.execute(
			invocation("commit_mutation", { operationId, approvalToken: approval.approvalToken }),
		);
		expect(committed.ok).toBe(true);
		expect((await x.store.getRecord("ISSUE", "i1", scope))?.data.status).toBe("DONE");
		const replay = await x.runtime.execute(
			invocation("commit_mutation", { operationId, approvalToken: approval.approvalToken }),
		);
	...
	it("rejects optimistic lock changes after approval", async () => {
		...
		const operationId = (prepared.data as { proposal: { operationId: string } }).proposal.operationId;
		const approval = await x.runtime.approve(operationId, context, true);
		await x.store.transaction(async (session) => {
			await session.update("ISSUE", "i1", { name: "Concurrent" }, "1", scope);
			return undefined;
		});
		const result = await x.runtime.execute(
			invocation("commit_mutation", { operationId, approvalToken: approval.approvalToken }),
		);
		expect(result.error?.code).toBe("MUTATION_VERSION_CONFLICT");
		expect((await x.store.getRecord("ISSUE", "i1", scope))?.data.status).toBe("OPEN");
	});
	it("uses soft delete by default and verifies reread", async () => {
		const x = setup();
		const prepared = await x.runtime.execute(
			invocation("prepare_delete", { entityType: "ISSUE", entityIds: ["i1"], reason: "duplicate" }),
		);
		const operationId = (prepared.data as { proposal: { operationId: string } }).proposal.operationId;
		const approval = await x.runtime.approve(operationId, context, true);
		const result = await x.runtime.execute(
		...
		expect(await x.store.getRecord("ISSUE", "i1", scope)).toBe(undefined);

...
describe("rag qa access", () => {
	it("enforces visibility, explicit ACL and security tags before retrieval", () => {
		...
		expect(
			isAccessible(
				document("d", { scope: { ...scope, ownerUserId: "u1", aclRoles: ["engineer"], securityTags: ["S1"] } }),
				chunk("k", "d", "x", {
					scope: { ...scope, ownerUserId: "u1", aclRoles: ["engineer"], securityTags: ["S1"] },
				}),
				filter,
			),
		).toBe(true);
		expect(
			isAccessible(
				document("d", { scope: { ...scope, ownerUserId: "u1", securityTags: ["S2"] } }),
				chunk("k", "d", "x", { scope: { ...scope, ownerUserId: "u1", securityTags: ["S2"] } }),
				filter,
			),
		).toBe(false);
		expect(
			isAccessible(
				document("d", { scope: { ...scope, ownerUserId: "u3", departmentId: "dep-2" } }),
				chunk("k", "d", "x", { scope: { ...scope, ownerUserId: "u3", departmentId: "dep-2" } }),
				filter,
			),
		).toBe(false);
	});

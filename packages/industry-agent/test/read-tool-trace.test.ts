...
describe("READ tool Trace integration", () => {
	it("records both successful and failed tool calls", async () => {
		...
		const runtime = new ReadToolRuntime({
			permissionService: new InMemoryReadToolPermissionService([{ ...scope, permissions: ["documents.list"] }]),
			...
			trace: {
				...
				recordToolEnd(id, _name, _output, isError) {
					events.push(`end:${id}:${isError}`);
				},
			},
		});

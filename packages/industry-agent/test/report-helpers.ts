			...
			charts: [
				{
					...
					datasetId: "progress",
					categoryColumn: "name",
					valueColumns: ["quantity"],
					evidenceIds: ["fact-1"],
				},
			],
			narrative: [
				{ sectionId: "summary", heading: "摘要", body: "项目当前数量来自权威数据。", evidenceIds: ["fact-1"] },
			],
			evidence: [
				{
					evidenceId: "fact-1",
...
function output(format: ReportFormat): Omit<ReportRendererOutput, "embeddedEvidenceIds"> {
	if (format === "EXCEL")
		return {
			content: new Uint8Array([0x50, 0x4b, 0x03, 0x04, 1]),
	...
	if (format === "CHART")
		return {
			content: new TextEncoder().encode('<svg xmlns="http://www.w3.org/2000/svg"><text>ok</text></svg>'),
	...
	return {
		content: new TextEncoder().encode("# 摘要\n项目当前数量来自权威数据。"),
...
export function renderer(format: ReportFormat, override?: Partial<ReportRendererOutput>) {
	return new StaticReportRenderer(format, `${format.toLowerCase()}-renderer-v1`, (input) => ({
		...output(format),
...
export function service(
	options: {
		...
		customRenderers?: ReturnType<typeof renderer>[];
		customLimits?: Partial<ReportLimits>;
	} = {},
) {
	...
	return {
		...
		service: new ReportService({
			permissions: new InMemoryReportPermissionService(options.allowed ?? true),
			renderers: new InMemoryReportRendererRegistry(renderers),

...
describe("M11 report limits", () => {
	...
	it("rejects a multi-format report when the combined artifact size exceeds the total limit", async () => {
		...
		const harness = service({ customRenderers: [excel, pdf], customLimits: { maxArtifactBytes: 100, maxTotalArtifactBytes: 30 } });
		await expect(harness.service.generate(request(["EXCEL", "PDF"]))).rejects.toMatchObject({ code: "REPORT_LIMIT_EXCEEDED" });
	});
});

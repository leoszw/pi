	...
	it("rejects cross-project datasets and RAG citations without source coordinates", async () => {
		...
		const badScope = {
			...
			model: {
				...
				datasets: [
					{
						...scoped.model.datasets[0]!,
						scope: { tenantId: "tenant-1", companyId: "company-1", projectId: "project-2" },
					},
				],
			},
		};
		...
		const badRag = {
			...rag,
			model: { ...rag.model, evidence: [{ ...rag.model.evidence[0]!, kind: "RAG_CITATION" as const }] },
		};
	...
	it("rejects derived evidence without a valid parent", async () => {
		...
		const bad = {
			...
			model: {
				...
				evidence: [
					{
						...base.model.evidence[0]!,
						evidenceId: "derived",
						kind: "DERIVED" as const,
						parentEvidenceIds: ["missing"],
					},
				],
				datasets: [{ ...base.model.datasets[0]!, evidenceIds: ["derived"] }],
				charts: [],
				narrative: [],
			},
		};

...
describe("M12 sandbox service", () => {
	it("produces a report through the read-only broker path", async () => {
		...
		const service = new SandboxAnalysisService({
			...
			planner: new ScriptedSandboxPlanner(
				"planner-v1",
				{ kind: "SANDBOX_REQUIRED", reason: "Need aggregation", usage: sandboxUsage() },
				{ program, usage: sandboxUsage() },
			),
			schemaDiscovery: new StaticSandboxSchemaDiscovery(sandboxSchema()),
			broker,
			executor: new ScriptedSandboxExecutor(async (_input, dataAccess) => {
				await dataAccess.read("q1");
				return { output: sandboxOutput(), attestation: sandboxAttestation(program.python) };
			}),
			verifier: new ScriptedSandboxVerifier("verifier-v1", {
				kind: "ACCEPT",
				summary: "Verified",
				usage: sandboxUsage(),
			}),
	...
	it("stops at mutation prepare and never commits", async () => {
		...
		const service = new SandboxAnalysisService({
			...
			planner: new ScriptedSandboxPlanner(
				"planner-v1",
				{ kind: "SANDBOX_REQUIRED", reason: "Need analysis", usage: sandboxUsage() },
				{ program, usage: sandboxUsage() },
			),
			schemaDiscovery: new StaticSandboxSchemaDiscovery(sandboxSchema()),
			...
			verifier: new ScriptedSandboxVerifier("verifier-v1", {
				...
				summary: "Owner should change",
				recommendation: {
					operation: "UPDATE",
					entityType: "BOQ_ITEM",
					targetEntityIds: ["90071992547409931234"],
					values: { owner: "张三" },
					evidenceQueryIds: ["q1"],
				},
				usage: sandboxUsage(),
		...
		const result = await service.run({
			goal: "Analyze and suggest owner change",
			context: sandboxContext(),
			requestedReportFormats: ["NARRATIVE"],
		});
